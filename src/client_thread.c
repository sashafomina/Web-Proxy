...
static bool make_get_header(int client_fd, char **full_host, char **path) {
  ...
  *path = NULL;
  ...
  if (prefix == NULL || url == NULL || version == NULL || strtok_r(NULL, " ", &saveptr)
      != NULL) {
...
void read_cache_data(int client_fd, buffer_t *cache_data) {
  uint8_t *msg = buffer_data(cache_data);
  for (ssize_t s = buffer_length(cache_data); s > 0; s -= BUFFER_SIZE ) {
    if (s >= BUFFER_SIZE) {
      write(client_fd, msg, BUFFER_SIZE);
      msg += BUFFER_SIZE;
    }
    else {
      write(client_fd, msg, s);
    }
  }
}
...
static bool send_response(int client_fd, int server_fd, char *key) {
  ...
  /* To be added to the cache upon EOF */
  buffer_t *web_data = buffer_create(BUFFER_SIZE);
  while (true) {
    ...
    /* Server sent EOF */
    if (bytes_read == 0) {
      /* Only add the new data to the cache if it is less than a max size*/
      if (buffer_length(web_data) <= MAX_OBJECT_SIZE) {
	cache_add(cache, key, web_data);
      }
      else {
	free(web_data);
      }
    ...
    /* Append read bytes into the web data object, so it can be cached later*/
    buffer_append_bytes(web_data, buf, bytes_read);
...
void *handle_request(void *cfd) {
  /* Detach threads, so that its resourc are automatically freed upon 
   * termination */
  ...
  /* Check if host and path is already in the cache*/
  /* Create the key by concatnating host and path */
  char *key = malloc((strlen(host) + strlen(path) + 1) * sizeof(char));
  strcpy(key, host);
  strcat(key, path);
  /* Get returns a copy so no cahce the web data will be deleted by 
   * another thread before it is written to the client */
  buffer_t *web_data = cache_get(cache, key);
  /* A cache hit */
  if (web_data) {
    write(client_fd, buffer_data(web_data), buffer_length(web_data));
    buffer_free(web_data);
    goto CACHE_HIT;
  }
